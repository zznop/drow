...
bool load_fmap(fmap_t **file, const char *filename)
{
    ...
    *file = NULL;
    int size = _get_file_size(filename);
    if (size < 0) {
    ...
    void *data = (void *)mmap(NULL, size, PROT_READ|PROT_WRITE, MAP_PRIVATE, fd, 0);
    if (!data) {
...
bool export_elf_file(fmap_t *elf, fmap_t *patch, char *outfile,
    struct tgt_info *tinfo, uint32_t old_entry, int inject_method)
{
    char *pad = NULL;
    size_t remaining;
    bool rv = false;
    ...
    int n = write(fd, elf->data, tinfo->base);
    if ((uint32_t)n != tinfo->base) {
        fprintf(stderr, ERR "Failed to export ELF (write 1st ELF chunk)\n");
        goto done;
    }

    uint32_t stager_size = (uintptr_t)&g_stager_end - (uintptr_t)&g_stager;
    ...
    *(uint32_t *)(stager_buf + 6) = tinfo->base;
    ...
    if (inject_method == METHOD_EXPAND_AND_INJECT) {
        size_t padsize = tinfo->size - patch->size - stager_size;
